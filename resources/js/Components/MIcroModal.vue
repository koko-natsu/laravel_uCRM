...
<template>
  ...
  <div v-show="isShow" class="modal" id="modal-1" aria-hidden="true">
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
      <div class="modal__container w-3/4 h-4/5" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
        ...
        <main class="modal__content" id="modal-1-content">
          ...
          <!-- 顧客リスト -->
            <table class="table-auto w-full text-left whitespace-no-wrap">
              ...
              <tbody>
                <tr v-for="customer in customers.value.data" :key="customer.id">
                  <td class="px-4 py-3">
                    <button type="button" @click="setCustomer({id:customer.id, name: customer.name})">{{ customer.id }}
                    </button>
                  </td>
            ...
            <!-- <Pagination class="mt-6" :links="customers.value.links"></Pagination> -->
          </div>
        </main>
  ...
  <input name="customer" v-model="search" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200">
  <button @click="searchCustomer" type="button" class="flex mt-5 mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">検索する</button>
</template>